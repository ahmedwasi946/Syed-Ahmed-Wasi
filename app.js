...
function initGlobal() {
  ...
  if (metaDesc) {
    metaDesc.setAttribute(
      "content",
      `${profile.name || "Developer"} - ${profile.title || ""} Portfolio`
    );
  }
...
function normalizePortfolioData() {
  ...
  profileLinkFields.forEach(field => {
    ...
    profile[field] =
      parseMarkdownLink(profile[field]).href;
  });

  if (profile.email) {
    const parsedEmail =
      parseMarkdownLink(profile.email);
  ...
  if (Array.isArray(portfolioData.projects)) {
    portfolioData.projects.forEach(project => {
      ...
      if (project.live) {
        project.live =
          parseMarkdownLink(project.live).href;
      }

      if (project.github) {
        project.github =
          parseMarkdownLink(project.github).href;
      }
    });
  }

  if (Array.isArray(portfolioData.codingProfiles)) {
    portfolioData.codingProfiles =
      portfolioData.codingProfiles
        .map(entry => {
          const rawHref =
            entry?.href ||
  ...
  const socialIcons = {
    ...
    github: `
      <svg xmlns="http://www.w3.org/2000/svg"
        ...
        0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0
        C6 2 5 2 5 2
        c-.28 1.15-.28 2.35 0 3.5
        -.73 1.02-1.08 2.25-1 3.5
        0 3.5 3 5.5 6 5.5
    ...
    mail: `
      <svg xmlns="http://www.w3.org/2000/svg"
        ...
        <path d="m22 7-8.97 5.7
          a1.94 1.94 0 0 1-2.06 0L2 7"/>
      </svg>
    `
  };
...
function initAbout() {
  ...
  if (
    ...
    html += `
      ...
      <div
        class="glass-hover card-shine card animate-fade-up"
        style="
...
function initExperience() {
                  ...
                  ${
                    exp.link
                      ? `
                        ...
                        ">
                          <a
                            href="${parseMarkdownLink(exp.link).href}"
                            target="_blank"
  ...
  function updateTimeline() {
    ...
    const timelineLine =
      experienceContent.querySelector(
        ".timeline-line"
      );
        ...
        ">
          ...
          <div>
            ...
            ">
              ...
              projects & technical opportunities
            </p>
